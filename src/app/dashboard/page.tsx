...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "posts" | "musicians" | "add">("users");
  ...
  return (
     <SidebarProvider>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="p-4">
          {activeTab === "users" && <UsersTab baseUrl={BASE_URL} adminEmail={ADMIN_EMAIL} />}
          {activeTab === "posts" && <PostsTab baseUrl={BASE_URL} />}
          {activeTab === "musicians" && <MusiciansTab baseUrl={BASE_URL}/>}
          {activeTab === "add" && <AddMusicianTab onSuccess={() => setActiveTab("musicians")} baseUrl={BASE_URL} />}
        </main>
      </div>
    </SidebarProvider>
  );
