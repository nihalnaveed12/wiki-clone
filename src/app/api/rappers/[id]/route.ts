...
export async function GET(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        if (!result.success) {
            const statusCode =
                result.error === "Rapper not found" ? 404 : 500;
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        if (contentType?.includes("application/json")) {
            ...
            if (formData.has("bio"))
                updateData.shortBio = formData.get("bio"); // Fallback for 'bio'

            // Socials
            if (formData.has("socials")) {
                updateData.socials = JSON.parse(
                    formData.get("socials") as string
                );
            } else {
                updateData.socials = {};
                ...
                if (formData.has("twitter"))
                    updateData.socials.twitter = formData.get("twitter");
            }
                ...
                if (
                    ...
                    updateData.definingProject = {
                        ...
                        year: formData.has("definingProjectYear")
                            ? parseInt(
                                  formData.get("definingProjectYear") as string,
                                  10
                              )
                            : undefined,
                    };
            ...
            if (formData.has("fansOf"))
                updateData.fansOf = parseArrayField(
                    formData.get("fansOf") as string
                );
